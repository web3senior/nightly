...
class Panel extends Controller
{
  ...
  function admin_access($o = false, $id = false, $val = false)
  {
    $tbl = ['admin', 'id'];
    $this->view->endpoint = __FUNCTION__;
    $this->baseLoad();
    if ($o == true && empty($this->_error)) {
      switch ($o) {
        ...
        case "update":
          ...
          else  $_POST['password'] = $_POST['password_hidden'];
          
          unset($_POST['password_hidden']);
          $this->model->command("update", $tbl, $_POST, $id);
          $this->header(__FUNCTION__);
          break;
        case "insert":
      ...
      if (isset($_GET['page']) && is_numeric($_GET['page'])) {
        $this->view->pg = $_GET['page'];
        $data = $this->model->admin($tbl, ($_GET['page'] - 1) * 10);
      } else {
        $this->view->pg = 1;
        $data = $this->model->admin($tbl, 0);
      }

      $this->view->title = 'دسترسی مدیریت';
      $this->view->data = [
        "data" => $data
      ];
      $this->viewRender(__FUNCTION__);
  ...
  function admin()
  {
    ...
    if (isset($_GET["mod"]) && $_GET["mod"] == "update") {
      ...
      $this->view->data = $this->model->command("fetch", [0 => "admin", 1 => "admin_id"]);
